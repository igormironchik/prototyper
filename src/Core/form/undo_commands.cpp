...
void UndoAddLineToPoly::undo()
{
    m_undone = true;

    TopGui::instance()->projectWindow()->switchToPolylineMode();

    auto *poly = dynamic_cast<FormPolyline *>(m_form->findItem(m_id));
    ...
    if (poly->countOfLines() == 1) {
        ...
        auto *lineItem = dynamic_cast<FormLine *>(m_form->createElement<FormLine>(m_id));
...
void UndoAddLineToPoly::redo()
{
    if (m_undone) {
        TopGui::instance()->projectWindow()->switchToPolylineMode();

        auto *lineItem = dynamic_cast<FormLine *>(m_form->findItem(m_id));

        if (lineItem) {
            const QLineF line = lineItem->line();

            m_form->deleteItems(QList<QGraphicsItem *>() << lineItem, false);

            auto *poly = dynamic_cast<FormPolyline *>(m_form->createElement<FormPolyline>(m_id));
...
UndoChangeLine::UndoChangeLine(Page *form,
                               const QString &id,
...
UndoChangePen::UndoChangePen(Page *form,
                             const QString &id,
                             const QPen &oldPen,
                             const QPen &newPen)
...
UndoChangeBrush::UndoChangeBrush(Page *form,
                                 const QString &id,
...
UndoChangeTextOnForm::UndoChangeTextOnForm(Page *form,
                                           const QString &id)
...
void UndoChangeTextOnForm::undo()
{
    m_undone = true;

    auto *text = dynamic_cast<FormText *>(m_form->findItem(m_id));
...
void UndoChangeTextOnForm::redo()
{
    if (m_undone) {
        auto *text = dynamic_cast<FormText *>(m_form->findItem(m_id));
...
UndoChangeTextWithOpts::UndoChangeTextWithOpts(Page *form,
                                               const QString &id,
...
void UndoChangeTextWithOpts::undo()
{
    m_undone = true;
...
void UndoChangeTextWithOpts::redo()
{
    if (m_undone) {
...
void UndoChangeTextWithOpts::setTextOpts(const Cfg::TextStyle &opts)
{
    auto *obj = dynamic_cast<FormObject *>(m_form->findItem(m_id));
...
UndoChangeCheckState::UndoChangeCheckState(Page *form,
                                           const QString &id)
...
void UndoChangeCheckState::undo()
{
    m_undone = true;
...
void UndoChangeCheckState::redo()
{
    if (m_undone) {
        auto *c = find();
...
FormCheckBox *UndoChangeCheckState::find() const
{
...
UndoDuplicate::UndoDuplicate(Page *form,
...
UndoChangeZ::UndoChangeZ(Page *form,
                         const ZAndIds &origZ,
                         const ZAndIds &newZ)
...
UndoEditPoly::UndoEditPoly(Page *form,
                           const QLineF &oldL1,
                           const QLineF &oldL2,
                           const QLineF &newL1,
                           const QLineF &newL2,

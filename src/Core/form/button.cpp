...
{
public:
    FormButtonPrivate(FormButton *parent,
                      const QRectF &rect)
        : q(parent)
        , m_rect(rect)
        , m_proxy(nullptr)
...
void FormButtonPrivate::init()
{
    ...
    m_proxy->setMinSize(QSizeF(m.boundingRect(QLatin1Char('a')).size()));
...
void FormButtonPrivate::setRect(const QRectF &rect)
{
    m_rect = rect;
...
void FormButtonPrivate::connectProperties()
{
    if (m_props) {
...
void FormButtonPrivate::disconnectProperties()
{
    if (m_props) {
...
FormButton::FormButton(const QRectF &rect,
                       Page *page,
                       QGraphicsItem *parent)
    : QGraphicsObject(parent)
    , FormObject(FormObject::ButtonType,
                 page)
    , d(new FormButtonPrivate(this,
                              rect))
{
    d->init();
}
...
void FormButton::paint(QPainter *painter,
...
void FormButton::setObjectPen(const QPen &p,
...
void FormButton::setObjectBrush(const QBrush &b,
                                bool pushUndoCommand)
{
...
Cfg::Button FormButton::cfg() const
{
    Cfg::Button c;
...
void FormButton::setCfg(const Cfg::Button &c)
{
...
Cfg::TextStyle FormButton::text() const
{
...
void FormButton::setText(const Cfg::TextStyle &c)
{
...
QRectF FormButton::boundingRect() const
{
    return d->m_rect;
}

void FormButton::setPosition(const QPointF &pos,
                             bool pushUndoCommand)
{
...
QPointF FormButton::position() const
{
    return pos();
}

QRectF FormButton::rectangle() const
{
    QRectF r = boundingRect();
...
void FormButton::setRectangle(const QRectF &rect,
                              bool pushUndoCommand)
{
...
void FormButton::resize(const QRectF &rect)
{
    d->setRect(rect);
...
void FormButton::moveResizable(const QPointF &delta)
{
    moveBy(delta.x(), delta.y());
}

QSizeF FormButton::defaultSize() const
{
    return QSizeF(MmPx::instance().fromMmX(10.0), MmPx::instance().fromMmY(5.0));
}

QWidget *FormButton::properties(QWidget *parent)
{
...
void FormButton::updatePropertiesValues()
{
    if (d->m_props) {
        d->disconnectProperties();
...
FormObject *FormButton::clone() const
{

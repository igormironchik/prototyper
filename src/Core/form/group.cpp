...
void FormGroupPrivate::init()
{
    ...
    m_topLeft.reset(new FormMoveHandle(c_halfHandleSize,
                                       QPointF(c_halfHandleSize * c_halfDivider, c_halfHandleSize * c_halfDivider),
                                       q,
                                       q->parentItem(),
                                       q->page(),
                                       Qt::SizeAllCursor));
    ...
    m_topRight.reset(new FormMoveHandle(c_halfHandleSize,
                                        QPointF(0.0, c_halfHandleSize * c_halfDivider),
                                        q,
                                        q->parentItem(),
                                        q->page(),
                                        Qt::SizeAllCursor));
    ...
    m_bottomRight.reset(
        new FormMoveHandle(c_halfHandleSize, QPointF(0.0, 0.0), q, q->parentItem(), q->page(), Qt::SizeAllCursor));
    m_bottomRight->setZValue(c_mostTopZValue);
    m_bottomRight->hide();

    m_bottomLeft.reset(new FormMoveHandle(c_halfHandleSize,
                                          QPointF(c_halfHandleSize * c_halfDivider, 0.0),
                                          q,
                                          q->parentItem(),
                                          q->page(),
                                          Qt::SizeAllCursor));
...
FormGroup::FormGroup(Page *page,
                     QGraphicsItem *parent)
    : QGraphicsItemGroup(parent)
    , FormObject(FormObject::GroupType,
                 page,
                 0)
...
void FormGroup::setCfg(const Cfg::Group &c)
{
...
QList<QGraphicsItem *> FormGroup::children() const
{
...
QRectF FormGroup::boundingRect() const
{
    return QGraphicsItemGroup::boundingRect().adjusted(-c_halfHandleSize * c_halfDivider,
                                                       -c_halfHandleSize * c_halfDivider,
                                                       c_halfHandleSize * c_halfDivider,
                                                       c_halfHandleSize * c_halfDivider);
}

void FormGroup::paint(QPainter *painter,
                      ...
                      QWidget *widget)
{
    ...
    if (isSelected() && !group()) {
        ...
        const QPointF p(option->rect.x() + option->rect.width() / 2.0 - d->m_center->halfOfSize(),
                        option->rect.y() + option->rect.height() / 2.0 - d->m_center->halfOfSize());

        d->m_center->setPos(mapToParent(p));

        d->m_center->show();

        const QPointF p1(option->rect.x(), option->rect.y());

        d->m_topLeft->setPos(mapToParent(p1));

        d->m_topLeft->show();

        const QPointF p2(option->rect.x() + option->rect.width() - d->m_topRight->halfOfSize() * 2.0, option->rect.y());

        d->m_topRight->setPos(mapToParent(p2));

        d->m_topRight->show();

        const QPointF p3(option->rect.x() + option->rect.width() - d->m_bottomRight->halfOfSize() * 2.0,
                         option->rect.y() + option->rect.height() - d->m_bottomRight->halfOfSize() * 2.0);
        ...
        const QPointF p4(option->rect.x(),
                         option->rect.y() + option->rect.height() - d->m_bottomLeft->halfOfSize() * 2.0);
...
void FormGroup::setPosition(const QPointF &pos,
...
QPointF FormGroup::position() const
{
    return pos() + QGraphicsItemGroup::boundingRect().topLeft();
}

QRectF FormGroup::rectangle() const
{
...
void FormGroup::setRectangle(const QRectF &rect,
                             bool pushUndoCommand)
{
    FormObject::setRectangle(rect, pushUndoCommand);
}

void FormGroup::handleMoved(const QPointF &delta,
...
void FormGroup::handleReleased(FormMoveHandle *handle)
{
    Q_UNUSED(handle)
...
template<class Elem,
         class Config>
inline void FormGroup::createElemWithRect(const Config &cfg)
{
...
FormObject *FormGroup::clone() const
{

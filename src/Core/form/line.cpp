...
void FormLinePrivate::placeChild()
{
    const QLineF l = q->line();

    const QPointF p = q->pos();

    m_h1->setPos(l.p1().x() - m_h1->halfOfSize() + p.x(), l.p1().y() - m_h1->halfOfSize() + p.y());

    m_h2->setPos(l.p2().x() - m_h2->halfOfSize() + p.x(), l.p2().y() - m_h2->halfOfSize() + p.y());

    m_move->setPos((l.p1().x() + l.p2().x()) / c_halfDivider - m_move->halfOfSize() + p.x(),
                   (l.p1().y() + l.p2().y()) / c_halfDivider - m_move->halfOfSize() + p.y());
}

void FormLinePrivate::createHandles()
{
...
FormLine::FormLine(Page *page,
                   QGraphicsItem *parent)
    : QGraphicsLineItem(parent)
    , FormObject(FormObject::LineType,
                 page,
                 0)
...
void FormLine::setCfg(const Cfg::Line &c)
{
    QLineF line;
    line.setP1(QPointF(MmPx::instance().fromMmX(c.p1().x()), MmPx::instance().fromMmY(c.p1().y())));
    line.setP2(QPointF(MmPx::instance().fromMmX(c.p2().x()), MmPx::instance().fromMmY(c.p2().y())));
...
QRectF FormLine::boundingRect() const
{
    return QGraphicsLineItem::boundingRect();
}

void FormLine::paint(QPainter *painter,
...
void FormLine::setObjectPen(const QPen &p,
...
void FormLine::showHandles(bool show)
{
...
QPointF FormLine::pointUnderHandle(const QPointF &point,
                                   bool &intersected,
                                   bool &intersectedEnds) const
{
    ...
    if (d->m_move->contains(d->m_move->mapFromScene(point))) {
        ...
        return QPointF((l.p1().x() + l.p2().x()) / c_halfDivider, (l.p1().y() + l.p2().y()) / c_halfDivider) + pos();
    }
...
bool FormLine::handleMouseMoveInHandles(const QPointF &point)
{
...
void FormLine::setPosition(const QPointF &pos,
                           bool pushUndoCommand)
{
...
QPointF FormLine::position() const
{
    return pos() + QGraphicsLineItem::boundingRect().topLeft();
}

QRectF FormLine::rectangle() const
{
...
void FormLine::setRectangle(const QRectF &rect,
                            bool pushUndoCommand)
{
    Q_UNUSED(rect)
...
void FormLine::handleMoved(const QPointF &delta,
...
void FormLine::handleReleased(FormMoveHandle *handle)
{
    d->m_handleMoved = false;

    if (isSelected()) {
        if (handle == d->m_move.get()) {
            page()->undoStack()->push(new UndoMove(page(), objectId(), position() - d->m_oldPos));
        } else {
            page()->undoStack()->push(new UndoChangeLine(page(), objectId(), d->m_oldLine, line()));
        }
...
void FormLine::mouseMoveEvent(QGraphicsSceneMouseEvent *event)
{
    QGraphicsLineItem::mouseMoveEvent(event);
}

void FormLine::mousePressEvent(QGraphicsSceneMouseEvent *event)
{
    QGraphicsLineItem::mousePressEvent(event);
}

void FormLine::mouseReleaseEvent(QGraphicsSceneMouseEvent *event)
{
    QGraphicsLineItem::mouseReleaseEvent(event);
}

void FormLine::placeHandles()
{
    d->placeChild();
}

FormObject *FormLine::clone() const
{

...
void FormCheckBoxPrivate::setRect(const QRectF &rect)
{
    const QRectF r = rect;
...
void FormCheckBoxPrivate::connectProperties()
{
    if (m_props) {
...
void FormCheckBoxPrivate::disconnectProperties()
{
    if (m_props) {
...
FormCheckBox::FormCheckBox(const QRectF &rect,
                           Page *page,
                           QGraphicsItem *parent)
    : QGraphicsObject(parent)
    , FormObject(FormObject::CheckBoxType,
                 page)
    , d(new FormCheckBoxPrivate(this,
                                rect))
{
    d->init();
}
...
void FormCheckBox::paint(QPainter *painter,
...
void FormCheckBox::draw(QPainter *painter,
                        const QPen &pen,
                        const QBrush &brush,
                        const QFont &font,
                        const QRectF &rect,
                        ...
                        int dpi)
{
    ...
    if (r.height() > boxHeight(dpi)) {
        r.setTopLeft(
            QPointF(r.topLeft().x() + pen.widthF(), r.topLeft().y() + (r.height() - boxHeight(dpi)) / c_halfDivider));
    ...
    if (isChecked) {
        ...
        painter->drawLine(QLineF(r.topLeft().x() + boxHeight(dpi) / c_halfDivider,
                                 r.bottomLeft().y() - o,
                                 r.topRight().x() - o,
                                 r.topRight().y() + o));
    }
...
qreal FormCheckBox::boxHeight(int dpi)
{
    if (!dpi) {
...
void FormCheckBox::setObjectPen(const QPen &p,
...
void FormCheckBox::setObjectBrush(const QBrush &b,
                                  bool pushUndoCommand)
{
...
Cfg::CheckBox FormCheckBox::cfg() const
{
    Cfg::CheckBox c;
...
void FormCheckBox::setCfg(const Cfg::CheckBox &c)
{
...
Cfg::TextStyle FormCheckBox::text() const
{
...
void FormCheckBox::setText(const Cfg::TextStyle &c)
{
...
QRectF FormCheckBox::boundingRect() const
{
    const QFontMetrics fm(d->m_font);

    const qreal dy = (fm.height() > d->m_rect.height() ? (fm.height() - d->m_rect.height()) / 2.0 : 0.0);

    return {0.0, -dy, d->m_width, d->m_rect.height() + dy * c_halfDivider};
}

void FormCheckBox::setPosition(const QPointF &pos,
                               bool pushUndoCommand)
{
...
QPointF FormCheckBox::position() const
{
    return pos();
}

QRectF FormCheckBox::rectangle() const
{
    QRectF r = boundingRect();
...
void FormCheckBox::setRectangle(const QRectF &rect,
                                bool pushUndoCommand)
{
...
void FormCheckBox::resize(const QRectF &rect)
{
    setPos(rect.topLeft());
    ...
    const qreal dy = (fm.height() > rect.height() ? fm.height() - rect.height() : 0.0);
...
void FormCheckBox::moveResizable(const QPointF &delta)
{
    moveBy(delta.x(), delta.y());
}

QSizeF FormCheckBox::defaultSize() const
{
    return {MmPx::instance().fromMmX(15.0), boxHeight()};
}

QSizeF FormCheckBox::minimumSize() const
{
    return QSizeF(boxHeight(), boxHeight());
}

QWidget *FormCheckBox::properties(QWidget *parent)
{
...
void FormCheckBox::updatePropertiesValues()
{
    if (d->m_props) {
        d->disconnectProperties();
...
QString FormCheckBox::typeName() const
{
    return tr("Check Box");
}

bool FormCheckBox::isChecked() const
{
    return d->m_checked;
}

void FormCheckBox::setChecked(bool on)
{
    d->m_checked = on;
...
FormObject *FormCheckBox::clone() const
{

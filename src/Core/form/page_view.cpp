...
{
public:
    PageViewPrivate(const Cfg::Page &cfg,
                    PageView *parent)
        : q(parent)
        , m_scene(nullptr)
        , m_form(nullptr)
...
void PageView::enableSelection(bool on)
{
...
qreal PageView::scaleValue() const
{
    return d->m_scale;
}

void PageView::setScaleValue(qreal s)
{
    d->m_scale = s;
    ...
    if (event->modifiers() & Qt::ControlModifier) {
        if (event->angleDelta().y() != 0) {
            ...
            if (qAbs(scale - c_minZoom) < c_maxZero || qAbs(scale - c_maxZoom) < c_maxZero) {
                ...
                ensureVisible(QRectF(event->position(), QSizeF(c_ensureVisibleRegion, c_ensureVisibleRegion)));

...
{
public:
    FormObjectPrivate(FormObject *parent,
                      FormObject::ObjectType t,
                      Page *form,
                      int props)
        : q(parent)
...
FormObject::FormObject(ObjectType t,
                       Page *parent,
                       int props)
    : d(new FormObjectPrivate(this,
                              t,
                              parent,
                              props))
{
...
QWidget *FormObject::properties(QWidget *parent)
{
...
void FormObject::updatePropertiesValues()
{
    if (d->m_props) {
...
FormObject::ObjectType FormObject::objectType() const
{
    return d->m_type;
}

const QString &FormObject::objectId() const
{
    return d->m_id;
}

void FormObject::setObjectId(const QString &i)
{
    d->m_id = i;
}

const QPen &FormObject::objectPen() const
{
    return d->m_pen;
}

void FormObject::setObjectPen(const QPen &p,
                              bool pushUndoCommand)
{
    if (pushUndoCommand) {
        d->m_form->undoStack()->push(new UndoChangePen(d->m_form, d->m_id, d->m_pen, p));
    }

    d->m_pen = p;
}

const QBrush &FormObject::objectBrush() const
{
    return d->m_brush;
}

void FormObject::setObjectBrush(const QBrush &b,
                                bool pushUndoCommand)
{
    if (pushUndoCommand) {
        d->m_form->undoStack()->push(new UndoChangeBrush(d->m_form, d->m_id, d->m_brush, b));
    }

    d->m_brush = b;
}

Page *FormObject::page() const
{
    return d->m_form;
}

void FormObject::postDeletion()
{
}

QSizeF FormObject::defaultSize() const
{
    return {1.0, 1.0};
}

QSizeF FormObject::minimumSize() const
{
    return defaultSize();
}

void FormObject::updateHandlesPos()
{
    setPosition(position(), false);
}

void FormObject::setPosition(const QPointF &pos,
                             bool pushUndoCommand)
{
    if (pushUndoCommand) {
...
void FormObject::setRectangle(const QRectF &rect,
                              bool pushUndoCommand)
{
    if (pushUndoCommand) {

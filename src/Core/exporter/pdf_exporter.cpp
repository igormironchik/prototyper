...
class PdfExporterPrivate : public ExporterPrivate
{
public:
    PdfExporterPrivate(const Cfg::Project &cfg,
                       PdfExporter *parent)
        : ExporterPrivate(cfg,
                          parent)
    {
    ...
    void fillDocument(QTextDocument &doc,
                      qreal dpi);
    //! Print document.
    void printDocument(const QTextDocument &doc,
                       QPdfWriter &pdf,
                       const QRectF &body);
...
void PdfExporterPrivate::fillDocument(QTextDocument &doc,
                                      qreal dpi)
{
    ...
    for (const Cfg::Page &form : m_cfg.page()) {
        ...
        c.insertText(QString(QChar::ObjectReplacementCharacter) + QStringLiteral("\n"), fmt);
...
void PdfExporterPrivate::printDocument(const QTextDocument &doc,
                                       QPdfWriter &pdf,
                                       const QRectF &body)
{
    ...
    while (block.isValid()) {
        ...
        for (; !it.atEnd(); ++it) {
            ...
            bool isObject = txt.contains(QChar::ObjectReplacementCharacter);
            isImage = isObject && it.fragment().charFormat().isImageFormat();
            isBreak = isObject && (it.fragment().charFormat().objectType() == c_pageBreakType);
        ...
        if (isBreak) {
            ...
            if (s.width() > body.size().width() || s.height() > body.size().height() - y) {
                s.scale(QSize(qRound(body.size().width()), qRound(body.size().height() - y)), Qt::KeepAspectRatio);
            }
...
PdfExporter::PdfExporter(const Cfg::Project &project)
    : Exporter(std::make_unique<PdfExporterPrivate>(project,
                                                    this))
{
}
...
void PdfExporter::exportToDoc(const QString &fileName)
{
